...
/**
 * Screen providing a utility to manually trigger various Android system errors and Firebase events.
 *
 * This screen is designed for QA testing and development of the **TestNexus Health Dashboard**
 * issue detection logic. It includes interactive buttons that can trigger:
 * - **Crashes**: NullPointerException, ArrayIndexOutOfBounds, StackOverflow, etc.
 * - **ANR (Application Not Responding)**: Explicitly blocking the main thread for 5+ seconds.
 * - **Memory Issues**: Allocating large byte arrays to trigger OutOfMemory (OOM).
 * - **Performance Degradation**: Janky UI through frame skips and GC pressure.
 * - **Firebase Integration**: Starting/stopping custom traces and fetching Remote Config.
 *
 * @param onNavigateBack Callback to return to the previous screen (typically the Home screen).
 *
 * @see triggerNullPointerException
 * @see triggerAnr
 * @see triggerOutOfMemory
 * @see triggerSkippedFrames
 */
...
fun TestErrorsScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            // ========== FIREBASE PERFORMANCE SECTION ==========
            SectionHeader("Firebase Performance Traces")

            val context = LocalContext.current

            ErrorButton(
                title = "Custom Trace — checkout_flow (2s)",
                description = "Start a 2-second custom trace called 'checkout_flow'",
                onClick = {
                    val trace = Firebase.performance.newTrace("checkout_flow")
                    trace.start()
                    trace.putAttribute("screen", "TestErrors")
                    trace.incrementMetric("items_count", 3)
                    Handler(Looper.getMainLooper()).postDelayed({
                        trace.stop()
                        Toast.makeText(context, "checkout_flow trace sent (2s)", Toast.LENGTH_SHORT).show()
                    }, 2000)
                },
                isDestructive = false
            )
            ...
            ErrorButton(
                title = "Custom Trace — image_upload (5s)",
                description = "Start a 5-second custom trace called 'image_upload'",
                onClick = {
                    val trace = Firebase.performance.newTrace("image_upload")
                    trace.start()
                    trace.putAttribute("file_type", "png")
                    trace.incrementMetric("file_size_kb", 2048)
                    Handler(Looper.getMainLooper()).postDelayed({
                        trace.stop()
                        Toast.makeText(context, "image_upload trace sent (5s)", Toast.LENGTH_SHORT).show()
                    }, 5000)
                },
                isDestructive = false
            )
            ...
            ErrorButton(
                title = "Network Request — Slow API",
                description = "Make a real HTTP request to simulate network latency",
                onClick = {
                    Thread {
                        try {
                            val url = java.net.URL("https://httpbin.org/delay/3")
                            ...
                            Handler(Looper.getMainLooper()).post {
                                Toast.makeText(context, "Network request done: $responseCode (3s delay)", Toast.LENGTH_SHORT).show()
                            }
                        } catch (e: Exception) {
                            Handler(Looper.getMainLooper()).post {
            ...
            SectionHeader("Firebase Remote Config")

            ErrorButton(
                title = "Fetch Remote Config",
                description = "Fetch latest Remote Config values and display them",
                onClick = {
                    val remoteConfig = Firebase.remoteConfig
                    val configSettings = remoteConfigSettings {
                        minimumFetchIntervalInSeconds = 0 // Always fetch fresh for testing
                    }
                    remoteConfig.setConfigSettingsAsync(configSettings)
                    remoteConfig.fetchAndActivate()
                        .addOnSuccessListener {
                            val featureFlag = remoteConfig.getString("feature_dark_mode")
                        ...
                        .addOnFailureListener { e ->
                            Toast.makeText(context, "Config fetch failed: ${e.message}", Toast.LENGTH_LONG).show()
                        }
                },
                isDestructive = false
            )
            ...
            ErrorButton(
                title = "Log Config Values",
                description = "Print all Remote Config keys to Logcat",
                onClick = {
                    val remoteConfig = Firebase.remoteConfig
                    val allKeys = remoteConfig.all
                    if (allKeys.isEmpty()) {
                        Toast.makeText(context, "No Remote Config values. Fetch first.", Toast.LENGTH_SHORT).show()
                    } else {
                        val summary = allKeys.entries.joinToString("\n") { "${it.key} = ${it.value.asString()}" }
                        android.util.Log.d("SampleApp", "Remote Config values:\n$summary")
                        Toast.makeText(context, "${allKeys.size} config values logged to Logcat", Toast.LENGTH_SHORT).show()
                    }
                },
                isDestructive = false
            )
...
private fun triggerAssertionError() {
    throw AssertionError("Test AssertionError from SampleAppNexus triggered at ${System.currentTimeMillis()}")
}
...
private fun triggerAnr() {
    // Block main thread for 15 seconds — well past the 5s ANR threshold
    Thread.sleep(15000)
}
