...
/**
 * MainActivity serves as the primary entry point for the SampleAppNexus application.
 *
 * It initializes the Material 3 theme and sets up the root navigation host using [AppNavigation].
 * The activity also enables edge-to-edge display support for a modern immersive UI.
 */
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Enable Crashlytics for debug builds (disabled by default in debug)
        val crashlytics = FirebaseCrashlytics.getInstance()
        crashlytics.isCrashlyticsCollectionEnabled = true
        crashlytics.setCustomKey("app_mode", "debug_testing")
        crashlytics.log("SampleAppNexus started — Crashlytics enabled")
        Log.d("SampleApp", "Crashlytics collection enabled: ${crashlytics.didCrashOnPreviousExecution()}")
        setContent {
            SampleAppNexusTheme {
                AppNavigation()
            }
...
/**
 * A comprehensive personal information form demonstrating the integration of Android Autofill.
 *
 * This composable uses both [ContentType] semantics for modern Android versions (11+)
 * and a custom [autofill] modifier (stubbed for demonstration) to provide hints to the 
 * Android Autofill framework. It covers a wide range of fields including names, 
 * contact details, address components, and identity numbers (CPF/SSN).
 *
 * @param modifier Modifier for the form container, typically used for padding or sizing.
 * @param onSubmit Callback triggered when the 'Submit' button is clicked, providing a [UserInfo] snapshot.
 *
 * @see UserInfo
 * @see ContentType
 */
